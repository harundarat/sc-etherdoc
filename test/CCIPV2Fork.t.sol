...
contract CCIPV2EthereumForkTest is Test {
    ...
    address private constant ETHEREUM_SEPOLIA_ROUTER = 0x0BF3dE8c5D3e8A2B34D2BEeB17ABfCeBaf363A59;
    address private constant ETHEREUM_SEPOLIA_LINK = 0x779877A7B0D9E8603169DdbD7836e478b4624789;
    ...
    function setUp() public {
        ...
        s_sender = new EtherdocSender(
            ETHEREUM_SEPOLIA_ROUTER, ETHEREUM_SEPOLIA_LINK, address(this), address(this), address(this), address(this)
        );
    ...
    function test_ethereumRouterSupportsMantleAndQuotesExtraArgsV3() external {
        if (!s_rpcAvailable) {
            vm.skip(true);
        }

        assertGt(ETHEREUM_SEPOLIA_ROUTER.code.length, 0);
        assertGt(ETHEREUM_SEPOLIA_LINK.code.length, 0);
        assertTrue(IRouterClient(ETHEREUM_SEPOLIA_ROUTER).isChainSupported(MANTLE_SEPOLIA_CHAIN_SELECTOR));
...
contract CCIPV2MantleForkTest is Test {
    ...
    uint64 private constant ETHEREUM_SEPOLIA_CHAIN_SELECTOR = 16_015_286_601_757_825_753;
    ...
    function setUp() public {
        ...
        s_sender.configureRemote(ETHEREUM_SEPOLIA_CHAIN_SELECTOR, RECEIVER, 500_000, true);
        s_documentId = s_sender.registerDocument(DOCUMENT_DIGEST, DOCUMENT_CID);
    }

    function test_mantleRouterSupportsEthereumAndQuotesExtraArgsV3() external {
        if (!s_rpcAvailable) {
            vm.skip(true);
        }
        ...
        assertTrue(IRouterClient(MANTLE_SEPOLIA_ROUTER).isChainSupported(ETHEREUM_SEPOLIA_CHAIN_SELECTOR));
        assertGt(s_sender.quoteFee(s_documentId, ETHEREUM_SEPOLIA_CHAIN_SELECTOR), 0);
    }
}
